...
- Review changes to both native helper directories, native build scripts, lifecycle scripts, and the lockfile before installing.
...
2. The separate create helper requests full Calendar access because write-only permission cannot select a specific real calendar. Its command surface performs one save and no read/update/delete, but a compromise inherits the broader OS permission.
...
4. Calendar creation is non-idempotent. After a timeout, inspect the target calendar before retrying or a duplicate may be created.
